# Step 04-03. 약/영양제 모듈 구현

> **🎯 목표**: 정확한 복용 시간 알림과 리필 관리를 제공하는 약 관리 시스템 구현
...
**의존성**: Step 03 완료 필수  
**병렬 가능**: Step 04-01, 04-02, 04-04, 04-05와 동시 진행 가능  
**예상 소요 시간**: 1일  
**난이도**: ⭐⭐⭐

### 이전 단계 요구사항
- ✅ Step 03 완료: 공통 엔진 (NotificationOrchestrator 특히 중요)
...
- Step 04-04 (자기관리 모듈)과 동시 진행 가능
- Step 04-05 (자기계발 모듈)과 동시 진행 가능
- 서로 독립적이므로 충돌 없음

### 이 단계가 필요한 이유
- Step 06 알림에서 정확한 시간 알림의 핵심
- 건강 관리라는 명확한 가치 제공
- 노인, 만성질환자에게 필수 기능
...
**주의**: 이 모듈은 의료 기기가 아닙니다. 단순 리마인더 앱입니다.

### 법적 제한사항

- ❌ **약물 상호작용 경고**: 하지 않음 (의료 면허 필요)
- ❌ **복용 지침 제공**: 하지 않음 (처방전 정보만 표시)
- ❌ **의학적 조언**: 절대 제공하지 않음
- ✅ **단순 알림**: 설정한 시간에 알림만

### 핵심 원칙

1. **정확성**: 의료 판단 제외, 리마인더 중심
2. **신뢰성**: 개인정보 보호 최우선 (로컬 암호화)
3. **안전성**: 민감한 건강 데이터는 로컬 저장 기본, 선택적 동기화
4. **간결함**: 복용 기록 및 리필 관리에만 집중
...
## 다음 단계
- **step-04-04-self-care-module.md**: 자기관리 모듈 구현
- **step-05-onboarding.md**: 템플릿 온보딩 시스템 구현
- 5분 이내 가치 체감 경험 설계
