...
> **🎯 목표**: 다정한의 핵심 데이터 구조를 TypeScript 타입과 Firestore 스키마로 완벽하게 설계
...
### 이전 단계 요구사항
- ✅ Step 01 완료: Expo 프로젝트 생성 및 Firebase 연동
...
이 단계를 완료하면:
- ✅ 모든 TypeScript 타입 정의 완료 (Task, LifeObject, User 등)
- ✅ Firestore 컬렉션 구조 확정
- ✅ 데이터 변환 유틸리티 구현 (Timestamp ↔ Date)
- ✅ 기본 데이터 검증 함수 작성
...
다정한의 데이터 모델은 4가지 핵심 원칙을 따릅니다:

1. **생활 객체 중심**: Task는 LifeObject(물건·공간)와 연결
2. **주기 자동화**: 완료 시 다음 due를 자동 계산
3. **개인화**: 실행 이력 기반 주기 조정
4. **오프라인 우선**: 로컬 데이터 + 자동 동기화
...
export interface SharedTemplate {
  ...
  // 템플릿 내용 (userId, id, nextDue 제외)
  cleaningTasks: Omit<CleaningTask, 'userId' | 'id' | 'nextDue'>[];
...
### Firestore 인덱스 요구사항 (템플릿)

```json
{
  "indexes": [
    {
      "collectionGroup": "sharedTemplates",
