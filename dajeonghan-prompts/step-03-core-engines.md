...
> **🎯 목표**: 모든 모듈이 공유하는 핵심 로직을 구현하여 코드 중복을 제거하고 일관된 동작 보장
...
### 이전 단계 요구사항
- ✅ Step 02 완료: TypeScript 타입 정의 및 Firestore 스키마

### 다음 단계
- **Step 04-00**: 기능 모듈 개요
- **Step 04-01**: 청소 모듈 구현 (병렬 가능)
- **Step 04-02**: 냉장고 모듈 구현 (병렬 가능)
- **Step 04-03**: 약 모듈 구현 (병렬 가능)
- **Step 04-04**: 자기관리 모듈 구현 (병렬 가능)
- **Step 04-05**: 자기계발 모듈 구현 (병렬 가능)

### 이 단계를 건너뛸 수 없는 이유
- Step 04의 모든 모듈(04-01~04-05)이 이 엔진들을 사용합니다
- 주기 계산, 우선순위, 미루기 로직의 핵심
- 코드 중복 방지 및 일관성 보장

## 📋 완료 기준

이 단계를 완료하면:
- ✅ 4개의 핵심 엔진이 구현됨 (RecurrenceEngine, PostponeEngine, PriorityCalculator, NotificationOrchestrator)
- ✅ LifeEngineService 통합 서비스가 작동함
- ✅ 단위 테스트가 통과함 (Jest)
- ✅ 모든 모듈에서 재사용 가능한 구조
...
**다정한**의 가장 큰 차별화 요소는 "공통 엔진"입니다. 청소, 냉장고, 약, 자기관리, 자기계발 모듈이 각자 주기/미루기 로직을 따로 구현하는 대신, 하나의 통합된 엔진을 공유합니다.
...
1. **코드 재사용**: 한 번 작성하면 모든 모듈에서 사용
2. **일관성**: 모든 모듈이 동일한 방식으로 동작
3. **유지보수**: 버그 수정 시 한 곳만 수정
4. **확장성**: 새 모듈 추가 시 엔진 그대로 사용
...
- **step-04-01-cleaning-module.md**: 청소 모듈 구현
- 공통 엔진을 활용한 도메인별 확장
